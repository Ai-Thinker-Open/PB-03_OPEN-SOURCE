...
#define DEFAULE_SCAN_MAX_NUM                    10
// Scan duration in ms
...
#define DEFAULT_ACTION_AFTER_LINK           ( GAPMULTI_CENTRAL_SDP ) ///( GAPMULTI_CENTRAL_MTU_EXCHANGE | GAPMULTI_CENTRAL_DLE_EXCHANGE | GAPMULTI_CENTRAL_SDP )
...
static void multiRoleEstablishCB( uint8 status,uint16 connHandle,GAPMultiRole_State_t role,uint8 perIdx,uint8* addr )
{
    if( status == SUCCESS )
    {
        ...
        {
            at_parameters.conn_slave_counter++;
