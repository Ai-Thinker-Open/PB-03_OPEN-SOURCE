...
void bleuart_Init( uint8 task_id )
{
    ...
    // Setup the GAP Peripheral Role Profile
    {
        ...
        uint16 desired_min_interval = 100;
        uint16 desired_max_interval = 200;
        uint16 desired_slave_latency = 2;
        ...
        GAPRole_SetParameter( GAPROLE_TIMEOUT_MULTIPLIER, sizeof( uint16 ), &desired_conn_timeout );
            ...
            GAPRole_SetParameter( GAPROLE_MAX_CONN_INTERVAL, sizeof( uint16 ), &desired_max_interval );
            GAPRole_SetParameter( GAPROLE_SLAVE_LATENCY, sizeof( uint16 ), &desired_slave_latency );
						GAPRole_GetParameter(GAPROLE_MIN_CONN_INTERVAL, &min_conn);
				}while(min_conn != desired_min_interval);
				#endif
    }
...
//        }
        ...
        if(at_get_dma_flag())  // for DMA PT
        {
            ...
            else
            {
				...
				osal_start_timerEx( bleuart_TaskID, BUP_OSAL_EVT_AT_AUTO_SLEEP,2000 );
