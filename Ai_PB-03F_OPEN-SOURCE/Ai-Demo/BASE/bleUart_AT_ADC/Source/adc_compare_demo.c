/**************************************************************************************************
    ...
    OF SUBSTITUTE GOODS, TECHNOAT_LOGY, SERVICES, OR ANY CLAIMS BY THIRD PARTIES
    ...
    if(channel_done_flag == adc_cfg.channel)
    {
        for(i=2; i<8; i++)
        {
            if(channel_done_flag & BIT(i))
            {
                ...
                if(ch!=0)
                {
                    AT_LOG("P%d %d mv ",ch,(int)(value*1000));
                }
                else
                {
                    AT_LOG("invalid channel\n");
                }
            }
        }
...
static void adc_Compare_MeasureTask( void )
{
    ...
    // //  AT_LOG("_______________ret = %08x ",ret);
